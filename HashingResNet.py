...
if __name__ == '__main__':
    ...
    model = HashingResNet(3, (1, 28, 28), 25).to(device)
    ...
    if not args.predict_only:
        ...
        best_acc = 0.
        print('[HashingResNet] Train begin!')

        print('[HashingResNet] Step 1: Pretrain the classification network.')
        for ep in range(1, args.epoch + 1):
            ...
            for i, batch in enumerate(train_loader):
                ...
                loss.backward()
                optimizer.step()
                print(f'\r[HashingResNet][Epoch {ep}/{args.epoch}] Training > {i + 1}/{total_train_steps} Loss: {loss.item():.3f}', end='')
            print()
            ...
            with torch.no_grad():
                ...
                for i, batch in enumerate(valid_loader):
                    ...
                    print(f'\r[HashingResNet][Epoch {ep}/{args.epoch}] Validating > {i + 1}/{total_valid_steps} ...', end='')
            ...
            if acc > best_acc:
                print(f'[HashingResNet][Epoch {ep}/{args.epoch}] *** New best! *** Accuracy: {acc:.4f}')
                path = join(args.save_path, 'HashingResNet.bin')
                if not os.path.exists(args.save_path):
                    os.makedirs(args.save_path)
                torch.save(model.state_dict(), path)
                best_acc = acc

        print('[HashingResNet] Step 2: Calculate class feature centers.')
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(unnoisy_loader):
                ...
                print(f'\r[HashingResNet] Calculating center > {i + 1}/{total_unnoisy_steps} ...', end='')
            class_centers /= class_count
            print('done.')

        print('[HashingResNet] Step 3: Finetune the model using the centers.')
        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr / 10)
    ...
    print('[HashingResNet] Test begin!')
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(test_loader):
            ...
            y_true.append(batch_y.data.numpy())
            y_pred.append(pred.data.cpu().numpy())
            print(f'\r[HashingResNet] Testing > {i + 1}/{total_test_steps} ...', end='')
        y_true = np.concatenate(y_true)
        y_pred = np.concatenate(y_pred)
        acc = accuracy_score(y_true, y_pred)
    ...
    # plt.show()
    plt.savefig('figures/HashingResNet.png')
