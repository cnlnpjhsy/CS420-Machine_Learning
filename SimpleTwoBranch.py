...
from models import SimpleTwoBranch
...
if __name__ == '__main__':
    ...
    model = SimpleTwoBranch(3, (1, 28, 28), 25).to(device)
    ...
    if not args.predict_only:
        ...
        best_acc = 0.
        print('[SimpleTwoBranch] Train begin!')
        for ep in range(1, args.epoch + 1):
            ...
            for i, batch in enumerate(train_loader):
                ...
                logits = model(batch_x_raw, batch_x_png, batch_x_raw_len)
                loss = loss_func(logits, batch_y)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                print(f'\r[SimpleTwoBranch][Epoch {ep}/{args.epoch}] Training > {i + 1}/{total_train_steps} Loss: {loss.item():.3f}', end='')
            print()
            ...
            with torch.no_grad():
                ...
                for i, batch in enumerate(valid_loader):
                    ...
                    print(f'\r[SimpleTwoBranch][Epoch {ep}/{args.epoch}] Validating > {i + 1}/{total_valid_steps} ...', end='')
            ...
            if acc > best_acc:
                print(f'[SimpleTwoBranch][Epoch {ep}/{args.epoch}] *** New best! *** Accuracy: {acc:.4f}')
                path = join(args.save_path, 'SimpleTwoBranch.bin')
                if not os.path.exists(args.save_path):
                    os.makedirs(args.save_path)
                torch.save(model.state_dict(), path)
                best_acc = acc

    print('[SimpleTwoBranch] Test begin!')
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(test_loader):
            ...
            y_true.append(batch_y.data.numpy())
            y_pred.append(pred.data.cpu().numpy())
            print(f'\r[SimpleTwoBranch] Testing > {i + 1}/{total_test_steps} ...', end='')
        y_true = np.concatenate(y_true)
        y_pred = np.concatenate(y_pred)
        acc = accuracy_score(y_true, y_pred)
    ...
    # plt.show()
    plt.savefig('figures/SimpleTwoBranch.png')
